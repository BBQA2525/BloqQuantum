...
break_df = make_combinations(first_two_rows, n - 1)
...
df_lst = []
for df in break_df:
	break_df_class_df = pd.concat([df, first_two_y_rows], axis=1)
...
for df in df_lst:
	array_lst = []
	for index, spins in df.iterrows():  # replace with X_train_scaled for full length data runs, first_two_rows for testing
...
directory_path = "./data/QRC_BQIM_LinearRegression_" + str(qpu_topology['type']) + "_" + date_string
...
##########################################################################################
# end of bloq_19.py, start of bloq_18.py
##########################################################################################
import numpy as np
...
#     if len(arr) % n != 0:
#         raise ValueError("Array length is not divisible by n")
...
if check_all_n(binary_y_arr, 1) == True:
	...
	cm_test = confusion_matrix(y_true=binary_y_arr, y_pred=binary_predictions)
