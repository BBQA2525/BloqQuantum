...
from minorminer import find_embedding, buscliq
...
from MinorEmbedding import generate_combinations
...
quantum_annealing(X_train_scaled, G1, J_val=0.5)
...
directory_path = "./data/QRC_CQIM_LinearRegression_" + str(qpu_topology['type']) + "_" + date_string
...
for key, array in array_lst.items():
	feature_vector_arr = array[:-1]
	list_feat_vecs.append(feature_vector_arr)
...
quantum_annealing(X_test_scaled, G1, J_val=0.5)

now = datetime.datetime.now()
date_string = now.strftime("%Y-%m-%d_%H-%M-%S")
directory_path_test = "./data/QRC_CQIM_LinearRegression_test_" + str(qpu_topology['type']) + "_" + date_string
...
for key, array in array_lst_test.items():
	feature_vector_arr = array[:-1]
	list_feat_vecs_test.append(feature_vector_arr)
...
for key, array in array_lst.items():
	y_vector_arr = array[-1]
	list_y_vecs_test.append(y_vector_arr)
...
if check_all_n(binary_y_arr_test, 1) == True:
	...
	cm_test = confusion_matrix(y_true=binary_y_arr_test, y_pred=binary_predictions)
