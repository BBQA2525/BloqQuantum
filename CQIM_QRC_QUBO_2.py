...
def linear_regression(Xtrain,Ytrain,Xtest,Ytest):
        ...
        # Calculate slope b
        b = sum((Xtrain-X_mean)*(Ytrain-Y_mean)) / sum(((Xtrain - X_mean)**2))

        # Calculate intercept a
        a = (Y_mean - (b*X_mean))

        # Prediction on training data + MSE
        Y_pred_train = a + (b * Xtrain)
        training_error = sum((Ytrain-Y_pred_train)**2)/len(Ytrain)

        # Prediction on test data + MSE
        Y_pred_test = a + (b * Xtest)
        test_error = sum((Ytest-Y_pred_test)**2)/len(Ytest)

        # Calculate R squared
        if Ytrain.all() == Y_mean.all():
        	R2 = 0
        else:
        	R2 = sum((Y_pred_train-Y_mean)**2)/sum((Ytrain-Y_mean)**2)

        return a, b, training_error, test_error, R2
        
a, b, training_error, test_error, R2 = linear_regression(X,binary_y_arr,X,binary_y_arr)

print(f"Your LR equation is: Y = {a} + {b}*X")
print(f"Training MSE: {training_error}, Test MSE: {test_error}")
print(f"R squared: {R2}")

def credit_risk_prediction(feature_vector_array):
        y = a + (b * feature_vector_array)
        ans_set = []
        for item in y:
        	ans_set.append(np.unique(np.round(item.mean())))
        return ans_set
        
print(credit_risk_prediction(X))
ans_set_lst = credit_risk_prediction(X)
